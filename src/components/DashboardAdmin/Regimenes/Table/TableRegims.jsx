...
const TableRegims = ({ isOpen, viewModal, activation, regime = [] }) => {
  ...
  const Activate = async (row) => {
    const res = await RequestService.delete(`/regime/${row.id}`);
    const { data } = await RequestService.get('/regime');
    activation(data);
    if (res.status === 201) {
      success('Regimen Activado');
    };
  };
  const Desactivate = async (row) => {
    ...
    activation(data);
    if (res.status === 201) {
      success('Régimen Inactivado');
    };
  };
  const confirmation = (row) => {
    if (row.isActive === false) {
      Swal.fire({
        title: '¿Desea activar este Régimem?',
        icon: 'warning',
      ...
      Swal.fire({
        title: '¿Desea desactivar este Régimem?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#00bac7',
        cancelButtonColor: '#f01473',
                  ...
                  <TableRow key={row.id}>
                    ...
                    {isOpen && (
                      <Modal className='ModalContainer'>
                        <EditRegimen row={regimeInfo} onClick={viewModal} />
                      </Modal>
                    )}
                  </TableRow>
                ))}
